...
int main(int argc,char * const *argv) {
  ...
  while ((opt = getopt(argc,argv,"dDvqQhHrRi:p:t:T:l:a:A:c:C:k:V:346eEsSnN0xXw:")) != opteof)
    switch(opt) {
      case '4': ipv4 = 1; break;
      case '6': ipv6 = 1; break;
      ...
      case 'z': crldir = optarg; break;
      case 'k': keyfile = optarg; break;
      case 'K': keyfileec = optarg; break;
  ...
  switch (ssl_certkey(ctx,certfile,0 /* XXX */, keyfile,keyfileec,passwd_cb)) {
  ...
  if (!ssl_ca(ctx,cafile,cadir,verifydepth,0))
  ...
  if (!ssl_crls(ctx,crldir))
    strerr_die2x(111,FATAL,"unable to initialize CRL dir");
