...
bomb:
  ...
  if (!ssl_shutdown_sent(ssl)) ssl_shutdown(ssl);
  if (!ssl_shutdown_pending(ssl)) ssl_shutdown(ssl);
  while ((r2 = ssl_shutdown(ssl)) == -1) {
    int maxloops = 10;

    if (maxloops-- == 0) break;
    r3 = SSL_get_error(ssl, r2);
    if ((r3 == SSL_ERROR_WANT_READ) || (r3 == SSL_ERROR_WANT_WRITE)) {
      usleep(100000);
      continue;
    }
    break;
  ...
  switch (r2) {
    case -1: usleep (1111); break;
    case  0: usleep (2222); break;
    case  1: usleep (3333); shutdown(wfd,2); break;
  }
  if (leftstatus != -1) close(fdleft);
  if (rightstatus != -1) close(fdright);
  errno = r;
